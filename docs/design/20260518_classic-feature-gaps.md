...
> 🎉 **Phase A-F 全部完成！** 累加目标模式（原版风格）已落地。剩余仅 Phase G 元系统（4 项含 #14 关卡布局模板化），属"按需"长期机制。
...
## 十二、下一步：Phase G 元系统（按需）

经典版差距已全部填平。**#20 成就 / #21 赌局已决策不做**。剩余 2 项：

1. **#19 无尽模式 / 挑战模式**（中）—— 通关后解锁，矿物循环刷新 + 难度递增 ← **当前在做**
2. **#14 关卡布局模板化**（大）—— 部分关卡固定布局（特殊缝隙/岩石屏障）
